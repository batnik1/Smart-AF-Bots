...
def assign_rack(orders):
    ...
    for order in orders:
        ...
        target = order[1]
        lst = []
        collection.update_one({"type": order[0]}, {
                              "$inc": {"quantity": -1*target}})
        for nobj in list(docker):
            ...
            for j in range(len(lst)):
                if lst[j][0] > target:
                    collection.update_one({"type": order[0], "shelves.shelf": lst[j][1]}, {
                                          "$inc": {"shelves.$.quantity": -1*target}})
                    ...
                    target -= lst[j][0]
                    collection.update_one({"type": order[0]}, {"$pull": {"shelves": {
                                          "shelf": lst[j][1], "quantity": lst[j][0]}}})
                    if lst[j][1] in racks_dict:
...
def gen_a_order():
    ...
    types_chosen = random.choices([0, 1], weights=[0.9, 0.1])
    fk = types_chosen[0]
    for type in types_chosen:
        # if type not in [0,1]:
        #   continue
        # if type==0:
        #   tf=random.choices([1,0],weights=[0.9,0.1])[0]
        #   if tf==0:
        #     continue
        # if type==1:
        #   tf=random.choices([1,0],weights=[0.1,0.9])[0]
        #   if tf==0:
        #     continue
    ...
    # if fk==0:
    human_counter=random.randint(0,m-1)
    # else:
    #   human_counter=random.randint(m,2*m-1)
  #  human_counter = random.randint(0, 2*m-1)
    ...
    if len(order) == 0:
        return "Nothing"
    sorting_random = (random.randint(0, 2*sorting_n-1),
                      random.randint(0, 2*sorting_m-1))
    # logger.info('New Order is Placed with Order ID: '+str(order_id)+' which consists of '+str(order))
    logger.info('New Order'+','+str(order_id)+',' +
                '-'+','+'-'+','+'New Order is Placed.')
    order_db.insert_one({"_id": order_id, "order_progress": 0, "ordered_quantity": sum,
                        "Target_Racks": racks, "human_counter": human_counter})
    order_history.insert_one(
        {"_id": order_id, "ordered": order, "address": sorting_random})
...
def add_items(count):
    ...
    for _ in range(count):
        ...
        tyk = [0, 1]
        type = random.choices(tyk, weights=[0.9, 0.1])[0]
        item_types_in_db.add(type)
        quantity = random.randint(1, 3)
        shelf=str((random.randint(0,m-1),random.randint(0,1), random.randint(0, 4), random.randint(0, 4)))
        # if type==0:
        #   shelf=str((random.randint(0,m-1),0, random.randint(0, 4), random.randint(0, 4)))
        # else:
        #   shelf=str((random.randint(0,m-1),1, random.randint(0, 4), random.randint(0, 4)))
        # shelf = str((random.randint(0, m-1), random.randint(0, n-1),
        #             random.randint(0, 4), random.randint(0, 4)))
        if collection.find_one({"type": type}):
            collection.update_one(
                {"type": type}, {"$inc": {"quantity": quantity}})
